...
from app.repositories.mastery_repo import save_mastery_points
...
def process_and_ingest_document(document_id: UUID, file_content: bytes, filename: str):
    ...
    try:
        ...
        # Step 6a: Store Raw Mastery Points (regex-extracted, always available)
        if raw_mastery_data:
            save_mastery_points(raw_mastery_data, document_id, source="regex")
        
        # Step 6b: Refine via Gemini LLM and store refined points
        if raw_mastery_data:

  ...
  group('QuizAttemptModel.toJson', () {
    test('serializes all fields correctly', () {
      final model = QuizAttemptModel(
        ...
        subjectKey: 'math',
        skillTag: 'algebra',
        attemptedAt: DateTime.parse('2024-06-01T09:00:00.000'),
        correctAnswers: 8,
        totalQuestions: 10,
        duration: const Duration(seconds: 720),
        passed: true,
        correctAnswerNumbers: [1, 2, 3],
      );

      final json = model.toJson();

      expect(json['id'], 'attempt-abc');
      expect(json['student_uid'], 'uid-student');
      expect(json['subject_key'], 'math');
      expect(json['skill_tag'], 'algebra');
      expect(json['correct_answers'], 8);
      expect(json['total_questions'], 10);
      expect(json['duration_seconds'], 720);
      expect(json['passed'], isTrue);
      expect(json['correct_answer_numbers'], [1, 2, 3]);
    });
  });
  ...
  group('fromJson → toJson roundtrip', () {
    test('round-trips without data loss', () {
      final original = {
        ...
        'subject_key': 'arabic',
        'skill_tag': 'reading',
        'attempted_at': '2024-05-20T15:30:00.000',
        'correct_answers': 6,
        'total_questions': 8,
        'duration_seconds': 480,
        'passed': true,
        'correct_answer_numbers': [2, 4, 6],
      };

      final model = QuizAttemptModel.fromJson(original);
      final serialized = model.toJson();

      expect(serialized['id'], original['id']);
      expect(serialized['subject_key'], original['subject_key']);
      expect(serialized['correct_answers'], original['correct_answers']);
      expect(serialized['total_questions'], original['total_questions']);
      expect(serialized['duration_seconds'], original['duration_seconds']);
      expect(serialized['passed'], original['passed']);
    });
  });
}
